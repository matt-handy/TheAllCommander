...
public class WindowsRegistryConfigurationAuditMacro extends AbstractAuditMacro {
	...
	public static final String READABLE_NAME = "Windows Registry Configuration Audit Macro";
	...
	public MacroOutcome processCmd(String cmd, int sessionId, String sessionStr) {
		...
		if (!response.contains(WindowsConstants.WINDOWS_NO_REGISTRY_KEY)) {
			hasFindings = true;
			outcome.addAuditFinding(
					"Warning: This machine has been configured to use Legacy LAPS. Consider upgrading to the modern implementation of LAPS.");
		}
		...
		if (response.contains("0x1")) {
			hasFindings = true;
			outcome.addAuditFinding("Warning: plaintext password storage appears to be enabled. Please confirm with "
					+ CHECK_PLAINTEXT_DIGEST_CMD);
		}

		sendCommand(CHECK_AUTOLOGON, sessionId, outcome);
		response = awaitResponse(sessionId, outcome);
		String elements[] = response.split(WindowsConstants.WINDOWS_LINE_SEP);
		for (String element : elements) {
			if (element.contains("AutoAdminLogon") && element.contains("1")) {
				hasFindings = true;
				outcome.addAuditFinding("Warning: AutoAdminLogon is enabled");
			}
		}

		if (!hasFindings) {
			outcome.addMacroMessage(READABLE_NAME + ": No findings");
		}
