...
public class ReconScriptGenerator implements Wizard {
			...
			if(line.startsWith("set_web_server_port")) {
				...
				for(int idx = 0; idx < maxInstr; idx++) {
					if(idx < winDesktop.size()) {
						winDesktopDeclarations.append("String bypass" + (idx + 1) + " = \"" + winDesktop.get(idx) + "\";" + System.lineSeparator());
					}else {
						winDesktopDeclarations.append("String bypass" + (idx + 1) + " = \"echo 'noop'\";" + System.lineSeparator());
					}
					if(idx < winServer.size()) {
						winDesktopDeclarations.append("bypass" + (idx + 1) + " = \"" + winServer.get(idx) + "\";" + System.lineSeparator());
					}else {
						winDesktopDeclarations.append("bypass" + (idx + 1) + " = \"echo 'noop'\";" + System.lineSeparator());
					}
					invocations.append("ps.AddScript(bypass" + (idx + 1) +  ");" + System.lineSeparator() + "ps.Invoke();" + System.lineSeparator());
				}
				...
				alive=false;
				OutputStreamWriterHelper.writeAndSend(bw,  file.toString());
	...
	private String getHelpMessage() {
		...
		sb.append("Welcome to the recon script generator. This generates a recon script that consolidates into a single C# executable the invocation of the most common test scripts needed for the course." + System.lineSeparator());
		sb.append("Many of the scripts are hosted on a remote host. To use this tool, use the commands set_lhost and set_web_server_port to tell the script where to find your scripts." + System.lineSeparator());
		sb.append("The script is designed to invoke an AMSI command. It also allows for separate AMSI bypass commands to be configured for Windows Desktop and Windows server lineages. Please see the pen300_study_tools/amsi.properties file." + System.lineSeparator());
		sb.append("The script will save all output to C:\\Windows\\Tasks by default, use set_local_recon_data_dir to overwrite this preference." + System.lineSeparator());
