...
A framework has been added to TheAllCommander server to allow for logging of telemetry data received from clients. This can be used for logging, or recording system utilization resources. telemHttpsAgentDef.py has an example implementation of an extension of httpsAgent that uses this functionality. Users can extend httpsAgent and implement a custom run loop to add polling and generation of telemetry data as needed.

# PyInstaller and Windows Service Installation

Please note: Windows service and PyInstaller support is currently in an alpha state and has not been rigorously tested yet.

To run one of the TheAllCommander daemons as an executable, pyinstaller can compile it as follows: pyinstaller.exe --onefile --hidden-import win32timezone httpsAgent.py

To run one one of TheAllCommander daemons as a service, a win32serviceutil.ServiceFramework needs to be supplied. A sample harness for the test telemetry service has been generated for user convenience, found in telemHttpsAgentService.py.

To compile the service:  pyinstaller.exe --onefile --runtime-tmpdir=. --hidden-import win32timezone telemHttpsAgentService.py

To install the service: dist\telemHttpsAgentService.exe install. The service can then be managed as a normal windows service, or it can be started and stopped with "start" and "stop" arguments to the telemHttpsAgentService.exe executable.

At present, subprocess creation does not work with TheAllCommander daemons running as a service. 
