...
class WindowsAVEnumeratorTest {
	...
	void testDetectsCommand() {
		WindowsAVEnumerator macro = new WindowsAVEnumerator();
		assertTrue(macro.isCommandMatch(WindowsAVEnumerator.COMMAND));
	...
	private class ClientStartCmdEmulator implements Runnable {
		...
		public void run() {
			while (alive) {
				...
				}else if(command.equalsIgnoreCase(WindowsAVEnumerator.WMIC_COMMAND)) {
	...
	void sendsCorrectCommands() {
		...
		MacroOutcome outcome = macro.processCmd(WindowsAVEnumerator.COMMAND, sessionId, null);
		assertEquals(2, outcome.getOutput().size());
		assertEquals("Sent Command: 'wmic /namespace:\\\\root\\SecurityCenter2 path AntiVirusProduct get * /value'", outcome.getOutput().get(0));
