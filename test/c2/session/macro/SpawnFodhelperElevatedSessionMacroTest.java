...
class SpawnFodhelperElevatedSessionMacroTest {
		...
		public void run() {
			while (alive) {
				...
				if (command == null) {
					...
					if (command.equalsIgnoreCase(SpawnFodhelperElevatedSessionMacro.CLIENT_GET_EXE_CMD)) {
	...
	void testNominalSendCommand() {
		...
		assertEquals("Sent Command: '" + SpawnFodhelperElevatedSessionMacro.CLIENT_GET_EXE_CMD + "'", outcome.getOutput().get(0));
