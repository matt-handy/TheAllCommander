...
class PythonIntegrationTest {
	...
	void testHTTPSBrokenConnections() {
		...
		if (thisOs == OS.WINDOWS) {
			...
			// TestProcessor.testDaemonConnection(TestConstants.PYTHON_HTTPSDAEMON_TEST_EXE,
			// true, false);
		} else if (thisOs == OS.MAC) {
			System.out.println("SOCKS5 not yet supported on Mac");
		}
	...
	void testDNSBrokenConnections() {
		...
		// Tests not yet validated on Linux
		if (thisOs == OS.WINDOWS) {
			// TestProcessor.testDaemonConnection(TestConstants.PYTHON_DNSDAEMON_TEST_EXE,
			// true, false);
		} else if (thisOs == OS.MAC) {
			System.out.println("SOCKS5 not yet supported on Mac");
		}
	...
	void testEmailDaemonNominal() {
		 ...
		 * EmailHelper.setup().getProperty(Constants.COMMSERVICES).contains(
		 * "EmailHandler")) { EmailHelper.flushC2Emails();
		 * TestProcessor.testDaemonConnection(TestConstants.PYTHON_SMTPDAEMON_TEST_EXE,
		 * false, true); }
		 */
	}
