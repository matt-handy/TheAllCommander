...
This UAC bypass utilizes the HKCU\Software\Classes\Folder\shell\opens\command registry key by writing an executable command which is launched in place of sdclt as high integrity. After a bug report from the author of this project, as of August 2024, Windows Defender effectively blocks malicious executables from leveraging this. For more information, please see the [Bugs and Disclosures](Bugs.md)    

However, Windows Defender is sometimes disabled and another EDR solution is used in place. It's still worth testing to make sure the issue is mitigated or detected on an as-deployed system.
